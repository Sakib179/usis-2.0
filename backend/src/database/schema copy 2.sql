...
CREATE TABLE Sections (
    ...
    UNIQUE INDEX idx_course_semester (course_code, semester)  -- Adding the unique index
);

-- section seat capacity
CREATE TABLE Section_Seat (
    course_code VARCHAR(6),
    semester VARCHAR(50),
    max_seats INT,
    PRIMARY KEY (course_code, semester),
    FOREIGN KEY (course_code, semester) REFERENCES Sections(course_code, semester)
        ON DELETE CASCADE
);
